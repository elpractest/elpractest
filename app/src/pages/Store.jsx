...
import { demoStoreCats, demoStoreProducts } from '../lib/demoData';

/**
 * STORE — storefront. Category tiles + hero + product grid (reference layout).
 * Real purchasable passes (via /api/student/purchasable-courses + the existing
 * StudentCheckout / Razorpay modal) take precedence; when the gateway is off or
 * nothing is purchasable it falls back to the design's demo products so the
 * populated layout is visible. "Add to cart" on demo items is a TODO stub.
 */
...
export default function Store() {
  ...
  return (
    <div style={{ padding: '16px 18px 24px', animation: 'fade-in .35s ease both' }}>
      ...
      <h2 style={{ margin: '0 0 12px', font: '700 16px var(--font-display)', color: 'var(--tx)' }}>{hasProducts ? 'Available passes' : 'Trending now'}</h2>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: '14px' }}>
          {demoStoreProducts.map((p) => (
            <div key={p.title} style={{ borderRadius: '18px', background: 'var(--card)', border: '1px solid var(--line)', boxShadow: 'var(--cardsh)', overflow: 'hidden' }}>
              <div style={{ height: '120px', background: p.grad }} />
              <div style={{ padding: '11px 12px 13px' }}>
                <div style={{ font: '700 12.5px/1.25 var(--font-body)', color: 'var(--tx)', minHeight: '32px' }}>{p.title}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '8px' }}>
                  <span style={{ font: '800 14px var(--font-display)', color: 'var(--tx)' }}>{p.price}</span>
                  <span style={{ font: '600 10px var(--font-body)', color: '#748099', textDecoration: 'line-through' }}>{p.mrp}</span>
                </div>
                <button onClick={() => alert('Cart is coming soon.')} style={{ width: '100%', marginTop: '10px', padding: '9px', border: `1px solid ${tint('gold').bd}`, borderRadius: '10px', background: tint('gold').bg, color: tint('gold').c, font: '800 12px var(--font-body)', cursor: 'pointer' }}>Add to cart</button>
              </div>
            </div>
          ))}
        </div>
