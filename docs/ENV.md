...
| `APP_URL` | API root base URL | `http://localhost:8000` | `https://api.pactest.live` | Infrastructure |
| `FRONTEND_URL` | React SPA base URL | `http://localhost:3000` | `https://app.pactest.live` | Infrastructure |
...
| `SESSION_DOMAIN` | Cookie domain | `.pactest.live` | `.pactest.live` | Preceded with `.` for root & subdomains |
| `SANCTUM_STATEFUL_DOMAINS` | SPA domains allowed stateful cookies | `app.pactest.live,localhost:3000` | `app.pactest.live` | Sanctum CSRF protection |
| `CORS_ALLOWED_ORIGINS` | Allowed CORS origins | `http://localhost:3000` | `https://app.pactest.live,https://pactest.live,https://www.pactest.live` | Required for Astro contact form & public course fetch |
...
| `MAIL_FROM_ADDRESS` | Sender email address | `"noreply@pactest.live"` | `"noreply@pactest.live"` | Domain MX/SPF/DKIM verified |
...
| `GOOGLE_REDIRECT_URI` | OAuth Redirect | `${APP_URL}/api/auth/google/callback` | `https://api.pactest.live/api/auth/google/callback` | Google Cloud Console |
...
| `FACEBOOK_REDIRECT_URI` | OAuth Redirect | `${APP_URL}/api/auth/facebook/callback` | `https://api.pactest.live/api/auth/facebook/callback` | Meta for Developers |
...
|---|---|---|---|---|
| `VITE_API_URL` | Base API URL | `""` (uses Vite proxy) | `https://api.pactest.live` | Leave empty in dev if using Vite proxy |
...
| Variable | Description | Dev Value | Production Example | Notes |
|---|---|---|---|---|
| `PUBLIC_API_URL` | Base API URL for course/contact fetches | `http://localhost:8000` | `https://api.pactest.live` | Read at build time for course SSG |
| `PUBLIC_SPA_URL` | Student App base URL for CTA links | `http://localhost:3000` | `https://app.pactest.live` | Link destination for "Login" & "Register" |
