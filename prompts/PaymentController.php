 ...
 *   key/value string columns. Replace paymentGatewayEnabled() below with
 *   whatever Phase 4C actually built if it differs.
...
class PaymentController extends Controller
{
    ...
    public function createOrder(Request $request): JsonResponse
    {
        ...
        if ($finalAmount === 0) {
            return $this->enrollForFree($user, $batch, $coupon);
        }
    ...
    protected function enrollForFree($user, Batch $batch, ?Coupon $coupon): JsonResponse
    {
        return DB::transaction(function () use ($user, $batch, $coupon) {
            $payment = Payment::create([
                'user_id' => $user->id,
                'batch_id' => $batch->id,
                'coupon_id' => $coupon?->id,
            ...
            if (class_exists(\App\Models\AuditLog::class)) {
                \App\Models\AuditLog::record('payment.captured', $payment);
            }
