 ...
 * - An AuditLog::record(string $event, $subject) helper exists elsewhere
 *   in the app (used the same way the activation-code redemption flow
 *   from Phase 3A logs its events). Swap the call below for whatever
...
class PaymentEnrollmentService
{
    ...
    public function confirmAndEnroll(int $paymentId, ?string $razorpayPaymentId = null, ?string $razorpaySignature = null): array
    {
        return DB::transaction(function () use ($paymentId, $razorpayPaymentId, $razorpaySignature) {
            ...
            // Swap for your app's actual audit-log call.
